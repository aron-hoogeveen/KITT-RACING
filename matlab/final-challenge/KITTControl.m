...
if (challengeA)% Challenge A: no waypoint
    ...
    if (~step2) % For turning testing purposes, step2 is omitted
        ...
        while (~finished)
            ...
            [x, y, callN] = KITTLocation(offlineLoc, turnEndPos, endpoint, 1, callN, 0, recordArgs, Fs);%the duration of this computation is variable
...
elseif (challengeA ~= true) % Challenge B: one waypoint
    ...
    while (~finished)
        % Continuously retrieve the audio location
        [x, y, callN] = KITTLocation(offlineLoc, turnEndPos, waypoint, 1, callN, 0, recordArgs, Fs);% The duration of this computation is variable
        plot(handles.LocationPlot, x, y, 'm+',  'MarkerSize', 5, 'linewidth',2); % Plot the location point on the map
    ...
    current_loc = KITTLocation(offlineLoc, turnEndPos, waypoint, 1, callN, 0, recordArgs, Fs);

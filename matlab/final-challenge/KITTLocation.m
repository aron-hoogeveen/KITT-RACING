...
if (offline)
    ...
    if (testCase == 0)
        ...
        if (floor(turnEndPos(1)) < endpoint(1))
            x_samp = [floor(turnEndPos(1)):(abs(floor(turnEndPos(1))-endpoint(1))/simPointsAmount):endpoint(1)];
            %disp("[floor(turnEndPos(1)):(abs(floor(turnEndPos(1))-endpoint(1))/simPointsAmount)-1:endpoint(1)]" + newline +...
            %    "[" + string(floor(turnEndPos(1))) + ":" + string(abs(floor(turnEndPos(1))-endpoint(1))/simPointsAmount) + ":" +...
            %    string(endpoint(1)) + "]");
        else
            x_samp = fliplr([floor(endpoint(1)):(abs(floor(endpoint(1))-turnEndPos(1))/simPointsAmount):turnEndPos(1)]);
        end
        if (floor(turnEndPos(2) < endpoint(2)))
            y_samp = [floor(turnEndPos(2)):(abs(floor(turnEndPos(2))-endpoint(2))/simPointsAmount):endpoint(2)];
        else
            y_samp = fliplr([floor(endpoint(2)):(abs(floor(endpoint(2))-turnEndPos(2))/simPointsAmount):turnEndPos(2)]);
        end

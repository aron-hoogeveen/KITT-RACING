...
if d == 2 && nmic == 5 && max(transmap,[],'all') ~= 0 % check if correction is possible
    ...
    [~,Ind] = min(addmap,[],'all','linear');    %  index of the minimum value
...
if check && d == 2 && nmic == 5 && max(transmap,[],'all') ~= 0
